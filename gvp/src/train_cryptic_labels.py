...
def main():
    # Prepare data
    trainset = simulation_dataset(BATCH_SIZE, FILESTEM, use_tensors=use_tensors) # batch size = N proteins
    ...
    for epoch in range(start_epoch, NUM_EPOCHS):
        ...
        predictions, mask = predict_on_xtals(model, xtal_val_ids)
...
def use_global_weights(y, positive_weight, negative_weight):    
    ...
    weights = [
        positive_weight if res_example
...
def choose_balanced_inds_constant_size(y, n_residues):
    ...
    '''
    pos_mask = np.array(y, dtype=bool)
    neg_mask = ~np.array(y, dtype=bool)
